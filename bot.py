...
async def start_token(m: Message):
    ...
    if not check_if:
        return await m.reply(f"You haven't joined @{CHANNEL_USERNAME} yet. Please join the channel and then send me the link again.\nThank you!",
            buttons=[
                [
                    Button.url("RoldexVerse", url=f"https://t.me/{CHANNEL_USERNAME}"),
                ],
                [
                    Button.url(
                        "ReCheck ♻️",
